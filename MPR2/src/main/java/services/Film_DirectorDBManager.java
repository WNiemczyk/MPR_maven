	...
	public Film_DirectorDBManager() {
		...
		try {
			...
			if (!Film_DirectorTableExists) {
				statement
						.executeUpdate("CREATE TABLE Film_Director(id_film int, id_director int, CONSTRAIN id_film_fk FOREIGN KEY id_film REFERENCES film(id), CONSTRAIN id_director_fk FOREIGN KEY id_director REFERENCES director(id)");
