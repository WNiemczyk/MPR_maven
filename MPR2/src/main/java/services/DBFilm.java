...
public class DBFilm {
	...
	public DBFilm() {
		...
		try {
			...
			findDirectorId = connection
					.prepareStatement("SELECT id FROM (SELECT id, surname AS name FROM director) WHERE name = ?");
	...
	public void addFilm(Film film) {
		...
		try {
			...
			addFilmStatement.setString(3, film.getStatus().toString()); // obiekt
			...
			findDirectorId.setString(1, d.getSurname());
	...
	public List<Film> getAllFilms() {
		...
		try {
			...
			while (rs.next()) {
				...
				List<Director> director = null;
				
				films.add(new Film(rs.getString("title"), director, rs.getInt("year"), status));
			}
